...
$$
\rho_{23}(t)
=\frac{1 }{2 } \left\{\left[P_t\ket{e}\bra{e} + \left(1-P_t \right)\ket{g}\bra{g} \right]^{\otimes 2} + \ket{g}\bra{g}^{\otimes 2} + c^2(t) \ket{e}\bra{g}^{\otimes 2} + \mathrm{H.c.} \right\}
$$
...
\begin{aligned}
\bar{F}
&=\frac{1 }{4\pi } \int_{\theta=0}^{\theta=\pi}\sin\theta\mathrm{d}\theta \int_{\phi=0}^{\phi=2\pi}\mathrm{d}\phi \sum_{k=1}^{4} \Bra{\varphi_3}\hat{U}_k^{(3)}\Bra{\mathrm{Bell}_k^{(12)}} \ket{\varphi_1}\bra{\varphi_1}\otimes \rho_{23}(t) \Ket{\mathrm{Bell}_k^{(12)}} \hat{U}_k^{(3)\dag} \ket{\varphi_3} \\
&=\frac{2+\left|c(t) \right|^2\left[\left|c(t) \right|^2 - 1 \right] + \mathrm{Re}\left\{c^2(t) \right\}}{3 } 
\end{aligned}
$$
...
- 他们将具有相同基矢的信息串保留下来 作为密码，将不同基矢的舍弃。
...
$$
S
=E(\vec{a}_1,\vec{b}_1)-E(\vec{a}_1,\vec{b}_3)+E(\vec{a}_3,\vec{b}_1)+E(\vec{a}_3,\vec{b}_3)
=-2\sqrt{2}
$$

$$
E(\vec{a}_i,\vec{b}_j)
=\Braket{\Psi|\left(\hat{\vec{\sigma}}^A\cdot\vec{a}_i \right)\left(\hat{\vec{\sigma}}^B\cdot\vec{b}_j \right)|\Psi}
=-\vec{a}_i\cdot\vec{b}_j
=-\cos\left(\phi_i^A-\phi_j^B \right)
$$

若发现 $\left|S \right|\leqslant \sqrt{2} $，则放弃本轮密钥分发。
...
$x_i\in\left\{0,1 \right\},i=0,1,2,\cdots,n-1 $

量子傅里叶变换把状态 $\ket{\psi}=\ket{x_{n-1}x_{n-2}\cdots x_0} $ 变换为：

$$
\frac{1 }{2^{n/2} } \left(\ket{0} + \mathrm{e}^{2\pi\mathrm{i}\left[0.x_{n-1} \right]}\ket{1} \right)\otimes \left(\ket{0} + \mathrm{e}^{2\pi\mathrm{i}\left[0.x_{n-2}x_{n-1} \right]} \right) \otimes \left(\ket{0}+\mathrm{e}^{2\pi\mathrm{i}\left[0.x_0,\cdots x_{n-2}x_{n-1}\right]} \right)
$$
