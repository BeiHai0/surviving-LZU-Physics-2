...
|:---:|:---:|:---:|
|$\Ket{\Phi^+} $|$a\ket{0}_3+b\ket{1}_3 $|$\hat{I}_3^{(3)} $|
|$\Ket{\Phi^-} $|$a\ket{0}_3-b\ket{1}_3 $|$\hat{\sigma}_z^{(3)} $|
|$10 $|$b\ket{0}_3+a\ket{1}_3 $|$\hat{\sigma}_x^{(3)} $|
|$11 $|$-b\ket{0}_3+a\ket{1}_3 $|$\mathrm{i}\hat{\sigma}_y^{(3)} $|

- Alice 通过经典信道告诉 Bob 她的测量结果。

- Bob 对粒子 $3 $ 执行相应的局域操作，使 $a\ket{0}_1+b\ket{1}_1 $ 态出现在 Bob 的粒子 $3 $ 上，即粒子 $3 $ 的状态变为 $a\ket{0}_3+b\ket{1}_3 $
...
$$
\bar{F}
\equiv \frac{1 }{4\pi } \int_{0}^{\pi} \sin\theta\mathrm{d}\theta \int_{0}^{2\pi} \mathrm{d}\phi \sum_{k=1}^{4} p_k \Braket{\varphi|\Psi_{\mathrm{out},k}^{(3)}} \Braket{\Psi_{\mathrm{out},k}^{(3)}|\varphi}
$$
