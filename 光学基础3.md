...
若 $\left[\hat{A},\left[\hat{A},\hat{B} \right] \right] = \left[\hat{B},\left[\hat{A},\hat{B} \right] \right] = 0 $，且 $\left[\hat{A}, \hat{B} \right] = \mathrm{const} $，则有：
...
\rho
&=\frac{\mathrm{e}^{-\beta \hat{H}} }{Z } \\
&=\sum_{n=0}^{\infty} \frac{\bar{n}^n }{(1 + \bar{n})^{n+1} } \ket{n}\bra{n} 
\end{aligned}
$$
...
$$
\bar{n}
=\mathrm{Tr}\left(\hat{a}^\dag \hat{a} \rho \right)
=\frac{1 }{\mathrm{e}^{\hbar\omega \beta} -1 } 
$$
