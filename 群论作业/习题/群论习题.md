...
在量子力学中，微观体系的状态可以用波函数来表示。波函数所在的集合可以被视作一个函数空间，也称为线性空间。这个空间包含所有可能的波函数（态矢量）。函数空间是一个希尔伯特空间，即完备的内积空间。
...
然而，阶为 $6 $ 的非阿贝尔群是存在的，如 $\mathrm{S}_3 $ 群，这与假设矛盾。
