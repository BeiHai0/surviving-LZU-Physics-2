...
\begin{aligned}
&\varphi(\vec{x},t) 
=\frac{1 }{4\pi \varepsilon_0 } \frac{q }{r-\vec{r}\cdot \vec{v}_q /c } = \frac{1 }{4\pi \varepsilon_0 } \frac{q }{s }  \\
&\vec{A}(\vec{x},t)
=\frac{\mu_0 }{4\pi } \frac{q \vec{v}_q  }{r-\vec{r}\cdot \vec{v}_q/c } = \frac{\mu_0 }{4\pi } \frac{q\vec{v}_q }{s } 
\end{aligned}
\right.
,~~\vec{r} \equiv \vec{x}-\vec{x}_q(t'),~~\vec{v}_q\equiv \vec{v}_q(t')=\dot{\vec{x}}_q(t'),~~s\equiv r-\vec{r}\cdot\vec{v}_q/c  \\
$$
...
考虑 $\vec{r}\equiv \vec{x}-\vec{x}_q(t') $ 的响应：

$t' $ 作为中间变量满足方程 $t'=t-|\vec{x}-\vec{x}_q(t') |/c $ ，根据隐函数微分法则，其响应 $\delta t' $ 应满足：

$$
\delta t'
=\delta t - \frac{1 }{c } \frac{\vec{r} }{r } \cdot \left[ \delta \vec{x} - \vec{v}_{q}\delta t' \right]
$$
...
$$
\delta t'
=\frac{r\delta t - \frac{\vec{r} }{c } \cdot\delta \vec{x} }{r-\vec{r}\cdot\vec{v}_q/c } 
$$

$$
\begin{aligned}
\delta \vec{r}
&=\delta\vec{x} - \vec{v}_q\delta t'
\end{aligned}
$$
...
\begin{aligned}
\delta r
=\delta \sqrt{\vec{r}\cdot\vec{r}}
=\frac{\vec{r} }{r } \cdot\delta \vec{r}
=-\frac{\vec{r}\cdot\vec{v}_q }{s }\delta t + \frac{\vec{r} }{s }\cdot\delta \vec{x}  
\end{aligned}
$$
...
\begin{aligned}
\delta s
&=\delta r - \left[(\delta \vec{r})\cdot\vec{v}_q + \vec{r}\cdot\delta \vec{v}_q \right] /c \\
&=
\end{aligned}
$$
