...
由于 $\cos\alpha_{j}\in \left[-1 , 1 \right] $，因此 $\cos\theta_{j\pm} \in \left[-1 , 1-2x^2 \right] $，设临界角 $\theta_c $ 满足：
...
平均每 $\pi \big/ \left([(N+1)/2] \right) $ 角度内就有一个 $\alpha_j $，因此 $\alpha $ 的零点密度函数，记为 $f(\alpha) $，满足：

$$
[(N+1)/2]f(\alpha)\mathrm{d}\alpha
=\mathrm{d}\alpha \bigg/ \left\{\pi \big/ \left([(N+1)/2] \right) \right\}
=\frac{[(N+1)/2]\mathrm{d}\alpha }{\pi } 
$$
...
\begin{aligned}
g(\theta)
&=\frac{f(\alpha)\mathrm{d}\alpha }{2\mathrm{d}\theta } \\
